...
class Batcher:
    ...
    def __init__(self, tokens: np.ndarray, block_size: int, tok: Tokenizer,
        ...
        is_action = (tokens >= tok.act_off) & (tokens < tok.act_off + 2)
        self.is_action = torch.from_numpy(is_action)
    ...
    def batch(self, bs: int):
        ...
        x = self.tokens[xi]
        y = self.tokens[xi + 1]
        mask = (~self.is_action[xi + 1]).float()   # supervise everything except action targets
...
def train(data_path: str, tier: str = "nano", iters: int = 3000, bs: int = 64,
          lr: float = 3e-3, warmup: int = 100, noise_aug: float = 0.0,
    ...
    batcher = Batcher(tokens, cfg.block_size, tok, device, seed=seed)
...
def _main() -> None:
    ...
    train(args.data, tier=args.tier, iters=args.iters, bs=args.batch_size, lr=args.lr,
          noise_aug=args.noise_aug, out=args.out, seed=args.seed)

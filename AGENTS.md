...
- **Collision is derived from geometry, NOT the model's status token** (`engine.collides`, mirrored
  ...
  - **DAgger partially works, then plateaus (`dagger.py` + `dagger_loop.py`, batched + bf16
    autocast; `train.py --init-from`).** One round (3k self-generated episodes relabeled by the
    geometry oracle) cuts the model's RAW-status phantom rate **1.00 → ~0.45** and fixes collision
    recall (anti-gap 72 → ~58 ≈ engine), one-step accuracy intact. But **iterating (rounds 2–3)
    does NOT compound** — it plateaus ~0.45 (n=100). So native death is much better but not
    solo-reliable; **the geometry guard stays authoritative** and the deployed model is unchanged
    (guard = 0.00 phantom on any model). Future angles in DEVLOG §7. (NB: batched rollout MUST run
    under bf16 autocast — fp32 at B=256 has no tensor-core path on the 5090 and is ~15× slower.)
